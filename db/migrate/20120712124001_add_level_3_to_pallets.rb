class AddLevel3ToPallets < ActiveRecord::Migration
  def self.up
    ...
    #PalletPurchasePositionAssignment.where("pallets.id IS NOT NULL").where("purchase_positions.id IS NOT NULL").includes(:purchase_position, :pallet).each do |pallet_purchase_position_assignment|
    ...
    #  puts "ID #{pallet_purchase_position_assignment.id} OK!"
    #end
  end
