  ...
  def export
    ...
    CSV.open("/Users/sufu/code/rails/fraefel/public/csv/file.csv", "wb") do |csv|
      Article.all.each do |article|
        ...
        csv << ["#{baan_orno}", "#{baan_cntn}", "#{baan_pono}", "#{article_depot}", "#{baan_loca}", "         #{article.article_code}                                ", "#{baan_clot}", "8", "#{article.stock_unit}", "#{baan_qstk}", "#{baan_qstr}", "#{baan_cstk}", "13", "14", "15", "#{baan_csts}", "#{Time.now.to_i}", "#{baan_recd}", "19", "20", "21"]
      end
    end
    redirect_to(:back)
  end
